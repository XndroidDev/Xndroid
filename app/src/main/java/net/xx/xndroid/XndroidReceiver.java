...
public class XndroidReceiver extends BroadcastReceiver {
    ...
    private void handleBattery(Intent intent){
        ...
        LogUtil.defaultLogWrite("info", "battery:" + level +"%, BatteryLow=" + AppModel.sDevBatteryLow);
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if(action == Intent.ACTION_BATTERY_CHANGED){
            ...
            LogUtil.defaultLogWrite("info", "screen on");
        }else if(action == Intent.ACTION_SCREEN_OFF){
            AppModel.sDevScreenOff = true;
            LogUtil.defaultLogWrite("info", "screen off");
        }
    }
}
