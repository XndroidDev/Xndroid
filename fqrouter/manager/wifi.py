...
P2P_CLI_PATH = home_path + '/wifi-tools/p2p_cli'
IW_PATH = home_path + '/wifi-tools/iw'
...
def backup_config_files():
    if not os.path.exists(home_path + '/backup'):
        os.mkdir(home_path + '/backup')
    backup_config_file(WPA_SUPPLICANT_CONF_PATH, home_path + '/backup/wpa_supplicant.conf.bak')
    backup_config_file(P2P_SUPPLICANT_CONF_PATH, home_path + '/backup/p2p_supplicant.conf.bak')
...
def backup_config_file(src, dst):
    try:
        if os.path.exists(dst):
            return
        if shell.USE_SU:
            content = shell_execute(home_path + '/../busybox cat %s' % src)
        else:
            with open(src) as f:
                content = f.read()
...
def restore_config_files():
    if os.path.exists(home_path + '/backup/wpa_supplicant.conf.bak'):
        shell_execute(
            '%s -p %s %s' % (CP_PATH, home_path + '/backup/wpa_supplicant.conf.bak', WPA_SUPPLICANT_CONF_PATH))
    if os.path.exists(home_path + '/backup/p2p_supplicant.conf.bak'):
        shell_execute(
            '%s -p %s %s' % (CP_PATH, home_path + '/backup/p2p_supplicant.conf.bak', P2P_SUPPLICANT_CONF_PATH))
            ...
            if os.path.exists(cmdline_path):
                with open(cmdline_path) as f:
                    ...
                    if 'supplicant' in cmdline:
                        ...
                        try:
                            shell_execute(home_path + '/../busybox ls -a -l /proc/%s/cwd' % pid)
                        except:
...
def get_wifi_chipset():
    ...
    if chipset:
        ...
        if chipset.endswith('5931'): # sdio:c00v037Ad5931
            return 'mtk', '5931'
        if chipset.find("wcnss") >= 0:
            return 'wcnss', 'unknown'
        if chipset.find("bcm") >= 0:
            return 'bcm', 'unknown'
        if chipset.find("ti") >= 0:
            return 'ti', 'unknown'
        if chipset.find("mtk") >= 0:
            return 'mtk', 'unknown'
...
def enable_ipv4_forward():
    ...
    if shell.USE_SU:
        shell_execute(home_path + '/../busybox echo 1 > /proc/sys/net/ipv4/ip_forward')
    else:
...
def get_wpa_supplicant_control_socket_dir(conf_path=WPA_SUPPLICANT_CONF_PATH):
    if shell.USE_SU:
        content = shell_execute(home_path + '/../busybox cat %s' % conf_path)
