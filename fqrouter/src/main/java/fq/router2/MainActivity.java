...
import fq.router2.life_cycle.ManagerProcess;
...
public class MainActivity extends Activity implements
    ...
    static private void getPermission(String[] permissions,Activity activity)
    {
        if(Build.VERSION.SDK_INT>=23) {
            ...
            if (preToDo.size() == 0)
                return;
            if (tip)
                Toast.makeText(activity,"please approve the permissino.", Toast.LENGTH_SHORT).show();
    ...
    private void regReceiver(){
        BroadcastReceiver receiver = new ConnectivityChangeReceiver();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        getPermission(sPermissions, this);
        regReceiver();
            ...
            if (ASK_VPN_PERMISSION == requestCode) {
                if (resultCode == RESULT_OK) {
                    if (LaunchService.SOCKS_VPN_SERVICE_CLASS == null) {
                        ...
                        updateStatus(_str(R.string.status_launch_vpn), 80);
                    ...
                    onHandleFatalError(_str(R.string.status_vpn_rejected));
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        addMenuItem(menu, ITEM_ID_EXIT, _str(R.string.menu_exit));
    ...
    private void openAbout() {
        ...
        web.loadUrl(_str(R.string.about_page));
        ...
        new AlertDialog.Builder(this)
                .setTitle(String.format(_str(R.string.about_info_title), LaunchService.getMyVersion(this)))
                .setCancelable(false)
                .setPositiveButton(R.string.about_info_share, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        intent.putExtra(Intent.EXTRA_SUBJECT, _str(R.string.share_subject));
                        if (null == shareUrl || shareUrl.trim().isEmpty()) {
                            shareUrl = "https://s3-ap-southeast-1.amazonaws.com/fqrouter/fqrouter-latest.apk";
                        }
                        intent.putExtra(Intent.EXTRA_TEXT, String.format(_str(R.string.share_content), shareUrl));
                        startActivity(Intent.createChooser(intent, _str(R.string.share_channel)));
                    }
                })
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    shareUrl = DnsUtils.resolveTXT(_str(R.string.share_url_domain));
                } catch (Exception e) {
    ...
    public void exit() {
        ...
        displayNotification(this, _str(R.string.status_exiting));
    }
...
//        if (isVpnMode) {
//            updateStatus(_str(R.string.status_acquire_vpn_permission), 75);
...
//        }
    ...
    public void onReady() {
        isReady = true;
        ActivityCompat.invalidateOptionsMenu(this);
        updateStatus(_str(R.string.status_ready), 100);
        displayNotification(this, _str(R.string.status_ready));
...
//                        updateStatus(_str(R.string.status_downloading) + " " + Uri.parse(upgradeUrl).getLastPathSegment(), 5);
...
//                .show();
    ...
    public void onDownloadFailed(final String url, String downloadTo) {
        ActivityCompat.invalidateOptionsMenu(this);
        onHandleFatalError(_str(R.string.status_download_failed) + " " + Uri.parse(url).getLastPathSegment());
    ...
    public void onDownloaded(String url, String downloadTo) {
        downloaded = true;
        ActivityCompat.invalidateOptionsMenu(this);
        updateStatus(_str(R.string.status_downloaded) + " " + Uri.parse(url).getLastPathSegment(), 5);
        onExiting();
        try {
            ManagerProcess.kill();
        } catch (Exception e) {
    ...
    public void onDownloading(String url, String downloadTo, int percent) {
        if (System.currentTimeMillis() % (2 * 1000) == 0) {
            displayNotification(this, _str(R.string.status_downloading) + " " + Uri.parse(url).getLastPathSegment() + ": " + percent + "%");
        }
        TextView textView = (TextView) findViewById(R.id.statusTextView);
        textView.setText(_str(R.string.status_downloaded) + " " + percent + "%");
    }
...
//        checkUpdate();
    ...
    public void onExiting() {
        ...
        TextView statusTextView = (TextView) findViewById(R.id.statusTextView);
        statusTextView.setText(_str(R.string.status_exiting));
    }
            ...
            new AlertDialog.Builder(this)
                    ...
                    .setNegativeButton(R.string.full_google_play_clean_and_open, new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialogInterface, int i) {
                            killGooglePlay();
                            try {
                                ShellUtils.sudo(sFqHome + "/../busybox", "rm", "-rf", "/data/data/com.android.vending/*");
                            } catch (Exception e) {
    ...
    private void killGooglePlay() {
        try {
            ShellUtils.sudo(sFqHome + "/../busybox", "killall", "com.android.vending");
        } catch (Exception e) {
